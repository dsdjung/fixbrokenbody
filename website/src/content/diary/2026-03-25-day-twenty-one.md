---
title: "Day Twenty-One"
date: "2026-03-25"
summary: "Tuesday — good sleep. 40-minute walk to coffee and supermarket. Kimbab for lunch."
tags: ["walking", "meals", "family"]
routine:
  morning:
    done: true
    note: "Good sleep"
  exercise:
    done: true
    note: "40-minute walk to coffee shop and supermarket"
  meal1:
    done: true
    note: "Kimbab for lunch"
  meal2:
    done: false
    note: ""
  sleep:
    done: true
    note: "Good sleep"
---

Day twenty-one. Tuesday. Three weeks in. Slept well. Had kimbab for lunch, then walked to the coffee shop and supermarket — about 40 minutes of walking. It was good to have some coffee time with the love of my life.

Been having apple and grapefruit every day. Trying to keep the fruit intake consistent.
